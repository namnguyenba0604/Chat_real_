...
export const ChatContextProvider = ({ children, user }) => {
  ...
  const [isUserChatRoomsLoading, setIsUserChatRoomsLoading] = useState(false);
  const [userChatRoomsError, setUserChatRoomsError] = useState(null);
...
*/
  ...
  useEffect(() => {
    const getUserChatRooms = async () => {
      setIsUserChatRoomsLoading(true);
      setUserChatRoomsError(null);

      if (user?._id) {
        const userId = user?._id;

        const response = await getRequest(`${baseUrl}/rooms/${userId}`);

        if (response.error) {
          return setUserChatRoomsError(response);
        }

        setUserChatRooms(response);
      }

      setIsUserChatRoomsLoading(false);
    };
