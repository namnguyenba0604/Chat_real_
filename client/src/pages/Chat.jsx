...
const Chat = () => {
  ...
  const { userChats, isUserChatsLoading, updateCurrentChat,userChatRooms,
        ...
        <Stack direction="horizontal" gap={4} className="align-items-start">
          <Stack className="messages-box flex-grow-0 pe-3" gap={3}>
            ...
            {userChatRooms?.map((chatRoom, index) => {
              return (
                <div key={index} onClick={() => updateCurrentChat(chatRoom)}>
                  <UserCard chatRoom={chatRoom} user={user} />
                </div>
              );
            })}
          </Stack>
